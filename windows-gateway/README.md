...
```bash
curl -H "X-Api-Key: secret" http://127.0.0.1:18080/health
```
...
curl -L \
  -H "X-Api-Key: secret" \
  "http://127.0.0.1:18080/download?fileId=<fileId>&fileName=test.pdf" \
  -o test.pdf
```
...
- Windows Firewall chỉ nên allow IP backend/K8s gọi TCP 18080.
...
```bash
curl -H "X-Api-Key: secret" http://WINDOWS_IP:18080/health
```

```bash
curl -L \
  -H "X-Api-Key: secret" \
  "http://WINDOWS_IP:18080/download?fileId=<fileId>&fileName=test.pdf" \
  -o test.pdf
```
